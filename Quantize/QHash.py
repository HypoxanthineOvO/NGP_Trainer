...
class QHashEncoding(Module):
    def __init__(self, n_input_dims: int, encoding_config: dict, Weight_Bits = 8, Feature_Bits = 8):
        super().__init__()
    ...
    def load_states(self, states: torch.Tensor):
        ...
        for i, size in enumerate(self.sizes):
            ...
            state_dict[f"grid.{i}.weight"] = Linear_Quantize(weight, self.WeightBits)
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        inputs = Linear_Quantize(inputs, self.FeatureBits)
        ...
        outputs = Linear_Quantize(outputs, self.FeatureBits)
        return outputs